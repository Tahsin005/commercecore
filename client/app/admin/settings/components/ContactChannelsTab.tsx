...
export function ContactChannelsTab() {
  ...
  const [deletingChannel, setDeletingChannel] = useState<ContactChannelItem | null>(null);
  ...
  const handleDeleteConfirm = () => {
    ...
    deleteChannelMut.mutate(deletingChannel.id, {
      onSuccess: () => {
      ...
      <div className="bg-white rounded-2xl border border-maroon-100 shadow-md p-6 space-y-6 animate-pulse">
        <div className="flex items-center justify-between border-b border-maroon-100 pb-4">
          <div className="space-y-2">
            <div className="h-5 w-52 bg-maroon-200/70 rounded-md" />
            <div className="h-3 w-72 bg-maroon-100/60 rounded" />
          </div>
        ...
        <div className="divide-y divide-maroon-100 border border-maroon-100 rounded-xl overflow-hidden">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="p-4 flex items-center justify-between bg-white">
              <div className="space-y-2">
                <div className="h-4 w-32 bg-maroon-200/60 rounded" />
                <div className="h-3 w-28 bg-maroon-100/60 rounded" />
              </div>
              <div className="flex items-center space-x-3">
                <div className="h-6 w-16 bg-maroon-100/70 rounded" />
                <div className="h-5 w-5 bg-maroon-200/60 rounded" />
                <div className="h-5 w-5 bg-maroon-200/60 rounded" />
              </div>
            </div>
          ))}
  ...
  return (
    <div className="bg-white rounded-2xl border border-maroon-100 shadow-md p-6 space-y-6">
        ...
        {contactChannels.map((ch) => (
          <div key={ch.id} className="p-4 flex items-center justify-between text-xs bg-white">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button onClick={() => openEditModal(ch)} className="p-1 text-maroon-700 hover:text-maroon-900 cursor-pointer">
      ...
      <DialogModal
        ...
        title={editingChannel ? "Edit Contact Channel" : "Add Contact Channel"}
      >
        <div className="flex items-center justify-between border-b border-maroon-100 pb-3">
          <h3 id="dialog-title" className="font-serif font-bold text-lg text-maroon-900">
            {editingChannel ? "Edit Contact Channel" : "Add Contact Channel"}
          </h3>
          <button onClick={() => setChannelModalOpen(false)} className="text-maroon-500 hover:text-maroon-800 cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 text-xs font-sans">
          <div>
            ...
            {errors.label && <p className="text-red-500 text-[11px] mt-1 font-medium">{errors.label.message}</p>}
          </div>
          <div>
            ...
            {errors.phoneNumber && <p className="text-red-500 text-[11px] mt-1 font-medium">{errors.phoneNumber.message}</p>}
          </div>
          <div>
          ...
          <button type="submit" disabled={createChannelMut.isPending || updateChannelMut.isPending} className="w-full py-2.5 bg-maroon-900 hover:bg-maroon-950 text-white font-semibold rounded-xl flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-50">
      ...
      <ConfirmDeleteModal
        ...
        description={`Are you sure you want to delete channel "${deletingChannel?.label || 'Untitled'}"?`}
        isDeleting={deleteChannelMut.isPending}
      />
    </div>
  );
}
