  ...
  const onSubmit = (data: BannerFormInput) => {
    if (editingBanner) {
      updateBannerMut.mutate(
        { id: editingBanner.id, data: { imageUrl: data.imageUrl, title: data.title, sortOrder: data.sortOrder } },
        { onSuccess: () => { toast.success("Banner updated!"); setBannerModalOpen(false); } }
      );
    } else {
      createBannerMut.mutate(
        { imageUrl: data.imageUrl, title: data.title, sortOrder: data.sortOrder, isActive: true },
        { onSuccess: () => { toast.success("Banner created!"); setBannerModalOpen(false); } }
      );
    }
  };
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-between text-[11px] pt-1">
                ...
                <div className="flex items-center space-x-2">
                  <button onClick={() => openEditModal(b)} className="text-maroon-700 hover:text-maroon-900">
                    <Edit2 className="w-3.5 h-3.5" />
                  </button>
                  <button
                    onClick={() => deleteBannerMut.mutate(b.id, { onSuccess: () => toast.success("Banner deleted") })}
      ...
      {bannerModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md space-y-4 border border-maroon-100 shadow-2xl relative">
            <button onClick={() => setBannerModalOpen(false)} className="absolute top-4 right-4 text-maroon-500 hover:text-maroon-800">
              <X className="w-5 h-5" />
            </button>
            <h3 className="font-serif font-bold text-lg text-maroon-900">{editingBanner ? "Edit Homepage Banner" : "Add Homepage Banner"}</h3>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-xs font-sans">
              <div>
                ...
                {bannerImageUrl ? (
                  <div className="relative h-40 w-full rounded-xl overflow-hidden border border-maroon-200 group bg-maroon-50">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-white text-maroon-900 font-semibold text-xs rounded-lg shadow-sm hover:bg-maroon-50 cursor-pointer flex items-center space-x-1"
                      >
                  ...
                  <div
                    onClick={() => bannerFileInputRef.current?.click()}
                    className="border-2 border-dashed border-maroon-200 hover:border-maroon-700 bg-off-white hover:bg-maroon-50/50 rounded-xl p-6 text-center cursor-pointer transition-all space-y-2 flex flex-col items-center justify-center"
                  >
                ...
                <div className="mt-2">
                  ...
                  />
                  {errors.imageUrl && <p className="text-red-500 text-[11px] mt-1">{errors.imageUrl.message}</p>}
                </div>
              </div>
