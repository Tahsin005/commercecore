    ...
    <div className="bg-maroon-900 p-6 sm:p-8 rounded-3xl text-cream shadow-md flex flex-col md:flex-row md:items-center justify-between gap-6 border border-maroon-800 max-w-full overflow-hidden">
      <div className="space-y-2 max-w-full min-w-0">
        <div className="flex items-center space-x-3 max-w-full min-w-0">
          <div className="w-12 h-12 rounded-2xl bg-maroon-800 border border-maroon-700 flex items-center justify-center text-white font-serif font-bold text-xl shadow-inner shrink-0">
            {user.name ? user.name.charAt(0).toUpperCase() : "U"}
          </div>
          <div className="min-w-0 flex-1">
            <h1 className="text-xl sm:text-2xl font-serif font-bold text-white tracking-wide truncate">
              {user.name || "Customer Account"}
            </h1>
            <p className="text-xs text-maroon-200 font-mono flex items-center gap-2 mt-0.5 max-w-full">
